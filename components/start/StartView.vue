<template>
  <v-ons-page class="start_view">
    <login-page v-on:kakaoLogin="KakaoLogin" v-on:unlinkKakao="unlinkKakao" v-on:next="temporary_start"></login-page>
  ...
  methods: {
    ...
    unlinkKakao: function (res) {
      let success = function () {
        window.alert("id : " + JSON.stringify(res) + "님의 카카오톡과 어플리케이션 연결이 끊어졌습니다.");
      };
      let failed = function (res) {
        window.alert("카카오톡 연결 해제 실패\n에러코드 : " + JSON.stringify(res));
      }
    ...
    KakaoLogin: function() {
      ...
      KakaoCordovaSDK.login(
        loginOptions, success, failed
      );
    }
  }
}
